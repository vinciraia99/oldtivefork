...
Sidebar.prototype.addGeneralPalette = function(expand)
{
	this.addPalette('general', mxResources.get('general'), expand || true, mxUtils.bind(this, function(content)
	{
	   ...
	   /* if (window.XMLHttpRequest)
        ...
        xhttp.open("GET","js/Line.xml",false);
        xhttp.send();
        console.log(xhttp.responseXML.getElementsByTagName("connector")[0].getElementsByTagName("values")[0]);*/
	   
	   
	   
	  // eval(xhttp.responseXML.getElementsByTagName("values")[0].getAttribute("code"));
...
Sidebar.prototype.addStencilPalette = function(id, title, stencilFile, style, ignore, onInit, scale)
{
	...
	this.addPalette(id, title, false, mxUtils.bind(this, function(content)
    {
		...
		/*console.log("oh");
		var v = mxUtils.load(stencilFile);
		root =v.getDocumentElement();
		packageName=root.getAttribute("name");
		
		console.log(root.getAttribute("name"));
		//packageName = v.getRoot(); 
		//console.log();
		...
		mxStencilRegistry.loadStencilSet(stencilFile, mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
		{
			if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
			{
				...
				/*var v = mxUtils.load(stencilFile);
				var v1=v.getDocumentElement();
				console.log(v1);*/
				//var v = mxUtils.createXmlDocument();
				/*var v = mxDomUtils();
...
//        			console.log(stringaCode);
	 ...
	 // eval(xhttp.responseXML.getElementsByTagName("connector")[0].getElementsByTagName("values")[0].getAttribute("code"));
    }));
};
