(function()
{
	...
	EditorUi.prototype.saveInput = function(name, saveAs)
	{
		...
		while(model.cells[i] != undefined )
		{
			
			if(model.cells[i].vertex)
			{
				model.cells[i].id="S_"+model.cells[i].id;
				symbols.push(model.cells[i]);
			}
			else if(model.cells[i].edge)
			{
				model.cells[i].id="C_"+model.cells[i].id;
				connectors.push(model.cells[i]);
			}
			i++;
		}
		console.log(symbols);
		...
		cellStates = this.editor.graph.getView().getStates().getValues();
		var grp = this.editor.graph;
		/*console.log('"'+symbols[0].mxObjectId+'"');
		console.log(cellStates.getKeys())
		console.log(cellStates.get('"'+symbols[0].mxObjectId+'"'));
		console.log(cellStates.getValues());*/
